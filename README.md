...
- 「Script files folder location」設定為 `90_Templates/scripts`（**這項一定要設定**，不是只有 Grok 功能才需要——批次產生骨架的程式邏輯放在這個資料夾，沒設定這項，快捷鍵會完全無法執行）
- 到 Templater 設定頁的「Template Hotkeys」區塊，新增一個項目指向 `_批次產生骨架.md`。這會在 Obsidian 全域「快捷鍵」設定裡自動產生兩個指令：「...: Insert ...」與「...: Create new note ...」。**只綁定 Insert 那個版本**（建議 `Ctrl+Alt+B`），不要綁 Create new note 版本——這個範本本身不輸出任何內容，用 Insert 才不會留下多餘的空白筆記。觸發前記得先打開任何一則現有筆記（哪一則都沒差，只是需要有作用中的分頁）
...
這個功能可以讓「批次產生骨架」在建立卡片時，自動呼叫 Grok（xAI）AI 幫每條劃線判斷主題標籤。**不設定也完全不影響其他功能**，只是卡片的 `tags` 欄位會統一填成「未分類」，需要自己手動改成真正的主題。
...
| 🐛 內化 | 建議每週固定時段 | 打開 `99_Dashboard.md`「已建骨架、尚未費曼轉譯」清單 → 選一張卡片打開 → 把標題與檔名改成你想到的概念名稱 → 手動撰寫費曼轉譯與個人經驗錨點 → 完成後把 `status` 改成 `done` |
...
- `90_Templates/`：Templater 與 Kobo 匯入的範本檔案；`90_Templates/scripts/` 放批次產生骨架與 Grok 分類的程式邏輯（`generate_stubs.js`、`stub_logic.js`、`tag_vocabulary.json`）——這個資料夾裡的檔案都會被 Templater 當成可執行的使用者腳本，不要把其他檔案放進去
- `tests/`：程式邏輯的自動化測試（純 Node.js，跟 Obsidian 執行環境無關），不是筆記內容
